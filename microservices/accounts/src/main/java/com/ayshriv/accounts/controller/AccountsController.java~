...
@Tag(
        name = "CRUD REST APIs for Accounts in ayshrivBank",
        description = "CRUD REST APIs in ayshrivBank to CREATE, UPDATE, FETCH AND DELETE account details"
)
public class AccountsController {
    ...
    @Operation(
            ...
            description = "REST API to create new Customer &  Account inside AyshrivBank"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    @GetMapping("/fetch")
    public ResponseEntity<CustomerDto> fetchAccountDetails(@RequestParam @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile number should be 10 digits")
                                                           String mobileNumber) {
